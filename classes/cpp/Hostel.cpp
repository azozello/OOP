...
void Hostel::saveStudents(vector<Student> students) {
    ...
    for (int i = 0; i < students.size(); ++i) {
        fout<<students[i].getName()<<" "<<students[i].getSurname() <<" "<<students[i].getGroup()
            <<" "<<students[i].getYear()<<" "<<students[i].getRoom().getNumber()<<endl;
    }
    fout.close();
}
...
void Hostel::saveStaff(vector<Staff> staff) {
    ...
    for (int i = 0; i < staff.size(); ++i) {
        fout<<staff[i].getName()<<" "<<staff[i].getSurname()<<" "<<staff[i].getJob()
...
void Hostel::saveTeachers(vector<Teacher> teachers) {
    ...
    for (int i = 0; i < teachers.size(); ++i) {
        fout<<teachers[i].getName()<<" "<<teachers[i].getSurname()
            <<" "<<teachers[i].getStatus()<<" "<<teachers[i].getRoom().getNumber()<<endl;

    }
    fout.close();
}

void Hostel::removeStudent(string name, string surname) {
    vector<Student> all = loadStudents();
    vector<Student> deleted;
    for (int i = 0; i < all.size(); i++) {
        if (all[i].getName()!=name && all[i].getSurname()!=surname){
            deleted.insert(deleted.end(),all[i]);
        }
    }
    saveStudents(deleted);
}

void Hostel::changeRoom(string name, string surname, int room) {
    vector<Student> all = loadStudents();
    vector<Student> newStudents;
    for (int i = 0; i < all.size(); i++) {
        if (all[i].getName()==name && all[i].getSurname()==surname){
            Student temp = all[i];
            temp.setRoom(getRoomByNumber(room));
            newStudents.insert(newStudents.end(),temp);
        } else {
            newStudents.insert(newStudents.end(),all[i]);
        }
    }
...
void Hostel::addStaff(Staff staff) {
    ...
    fout<<staff.getName()<<" "<<staff.getSurname()<<" "<<staff.getJob()<<" "<<staff.getSalary()<<endl;
    fout.close();
}
...
void Hostel::addTeacher(Teacher teacher) {
    ...
    fout<<teacher.getName()<<" "<<teacher.getSurname()<<" "<<teacher.getStatus()<<" "<<teacher.getRoom().getNumber()<<endl;
    fout.close();
}
...
void Hostel::addStudent(Student student) {
    ...
    fout<<student.getName()<<" "<<student.getSurname()<<" "<<student.getGroup()<<" "<<student.getYear()<<" "<<student.getRoom().getNumber()<<endl;
    fout.close();
}
